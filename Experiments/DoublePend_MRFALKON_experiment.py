...
from MRFALKON.MRFALKONmain import MRFALKON_Master, MRFALKONconfig
...
def run_double_pendulum_prediction(max_lvl, recording_length, state_ts_bhs, forecastStep, dpModelConfig, directory):
    ...
    for test_pend_nr in range(0, num_test_pendulums):
        ...
        for lvl in range(0, max_lvl+1):
            ...
            for step in range(1, max_step):
                fstate_ts_pred = mrfalkon.predict(fstate_ts_pred, max_lvl=lvl)
    ...
    mrfalkon.store_model_summary(directory, avg_mse_ts, ExperimentName)
...
if __name__ == '__main__':
    ...
    points = np.reshape(state_tr_bhs, newshape=(-1, 4))
    # Select landmarks from the training batches
    n_lm = int(lm_factor * np.sqrt(points.shape[0]))
    lm_idx = np.random.randint(0, points.shape[0], size=n_lm)
    landmarks = points[lm_idx]
    ...
    mrfalkon = MRFALKON_Master(initRadius, landmarks, mrfalkonconfig, ExperimentName)
    ...
    ### Train the model
    start = perf_counter()
    mrfalkon.learnFromStream(dataLoader=dataLoader, ExpName=ExperimentName, logg=True, maxLvl=None)
    stop = perf_counter()
