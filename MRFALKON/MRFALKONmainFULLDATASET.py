...
class MRFALKON_FULLDATASET_Master():
    ...
    def learnFromStream(self, dataLoader, ExpName, logg=False, maxLvl=None):
        ...
        self.stream = True
        while True:
            X_batch, y_batch, self.stream = dataLoader.streamData(self.batch_nr)
            ...
            self.batch_nr += 1
            print(f'Main Batch nr: {self.batch_nr}')
            ...
            #self.collectTrainingData(X_batch, y_batch, dataLoader)
            for x, y in zip(X_batch, y_batch):  ### Added all this as a test
                ...
                self.collectTrainingData(x, y, dataLoader)

    def collectTrainingData(self, xtr, ytr, dataLoader):
        lvl = self.curr_lvl
        ...
        if self.batch_nr >= dataLoader.get_n_batches() and self.curr_lvl <= self.maxLevel:
            print(" ")
            print(f"Lvl {lvl} is ready")
            print(f"Batch nr {self.batch_nr} of {dataLoader.get_n_batches()}")
            print(f"Number of training points")
            ...
            if self.collectionCounter % 100000 == 0:
                print(f"Waiting for training points, Num. training points at lvl "
                        f"{lvl} is {self.n_trp_lvl[f'lvl{lvl}']}")
        return
